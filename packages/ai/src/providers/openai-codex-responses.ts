/**
 * @fileoverview OpenAI Codex Responses API 提供商实现。
 *
 * 基于 OpenAI Responses API 适配 Codex 系列（如 codex-mini）的 Agent 模式：
 *   - 支持 Codex 特有的 reasoning 模式和工具调用
 *   - 与 openai-responses.ts 共享基础逻辑
 *   - 处理 Codex 特有的流式事件格式
 */
