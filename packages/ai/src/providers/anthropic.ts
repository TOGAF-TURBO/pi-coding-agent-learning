/**
 * @fileoverview Anthropic Messages API 提供商实现。
 *
 * 通过 Anthropic SDK 将 pi-ai 的标准接口适配到 Claude 的 Messages API：
 *   - 消息格式转换（pi Message → Anthropic MessageParam）
 *   - 工具定义转换
 *   - 流式响应解析（SSE 事件→AssistantMessageEvent）
 *   - 思考（Extended Thinking）支持
 *   - 缓存控制（Cache Control）
 *   - 完成/错误处理
 */
