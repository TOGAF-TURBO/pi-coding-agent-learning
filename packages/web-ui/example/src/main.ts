/**
 * @fileoverview Web UI 示例 — 演示如何嵌入和使用 pi-web-ui 组件。
 */
...
// Register GLM as a custom provider if not already present
async function ensureGlmProvider() {
	const providers = await customProviders.getAll();
	if (providers.some((p) => p.name === "glm")) return;
...
async function initApp() {
	...
	// Register GLM as a custom provider
	await ensureGlmProvider();
