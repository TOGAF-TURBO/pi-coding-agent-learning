/**
 * @fileoverview AgentInterface — Web UI 的核心聊天界面组件。
 *
 * 基于 LitElement 的 Web Component，提供：
 *   - 消息列表渲染（用户、助手、工具调用/结果）
 *   - 消息输入编辑器
 *   - 模型选择器
 *   - 流式响应处理
 *   - API Key 管理（支持 customProviders 存储）
 *   - CORS 代理配置
 */
	...
	private setupSessionSubscription() {
		...
		if (!this.session.getApiKey) {
			this.session.getApiKey = async (provider: string) => {
				// First check known provider keys
				const key = await getAppStorage().providerKeys.get(provider);
				if (key) return key;

				// Then check custom provider API keys
