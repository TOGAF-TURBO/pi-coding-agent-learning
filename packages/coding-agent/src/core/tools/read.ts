/**
 * @fileoverview Read 工具 — 读取文件内容，支持文本文件和图片。
 *
 * 功能：
 *   - 读取文本文件（支持 offset/limit 分页，截断时提示继续读取）
 *   - 读取图片文件（自动检测 MIME 类型，可自动缩放到 2000x2000 以内）
 *   - 输出截断（行数/字节数限制，超出时提示使用 offset 继续）
 *   - TUI 渲染：调用显示、语法高亮、折叠/展开、截断提示
 *   - 特殊分类：技能文件、文档文件、资源文件使用紧凑渲染
 *
 * 可扩展性：通过 ReadOperations 接口可替换为远程文件读取（如 SSH）
 */
