/**
 * @fileoverview Edit 工具 — 精确编辑文件内容（原地修改）。
 *
 * 功能：
 *   - 通过 oldText/newText 对精确替换文件内容
 *   - 支持多次替换（单个调用中的 edits 数组）
 *   - 差异计算和预览（diff 渲染）
 *   - 行尾规范化（LF/CRLF 自动检测和保持）
 *   - BOM 处理
 *   - 通过 withFileMutationQueue 确保文件变更的顺序执行
 *
 * 可扩展性：通过 EditOperations 接口可替换为远程文件编辑（如 SSH）
 */
