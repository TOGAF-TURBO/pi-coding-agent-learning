/**
 * @fileoverview Extension system types — pi 扩展系统的完整类型定义。
 *
 * 扩展（Extension）是 TypeScript 模块，可以通过统一的 API：
 *   - 订阅 Agent 生命周期事件（会话、消息、工具、模型等）
 *   - 注册 LLM 可调用的工具（Tool）
 *   - 注册命令（Command）、快捷键（Shortcut）、CLI 标志（Flag）
 *   - 与用户交互（选择器、确认框、输入框、通知等 UI 原语）
 *   - 注册自定义提供商和模型
 *
 * 类型分为以下几类：
 *   - UI 上下文：ExtensionUIContext（用户交互接口）
 *   - 扩展上下文：ExtensionContext / ExtensionCommandContext（事件和命令处理器的参数）
 *   - 工具类型：ToolDefinition（工具定义）+ ToolRenderContext（渲染上下文）
 *   - 事件类型：按领域分类（资源、会话、Agent、模型、Bash、输入、工具）
 *   - 事件结果：各事件处理器可返回的结果类型
 *   - 扩展 API：ExtensionAPI（暴露给扩展工厂函数的接口）
 *   - 提供商注册：ProviderConfig / ProviderModelConfig
 *   - 运行时类型：ExtensionRuntime / Extension（加载器和运行时的内部类型）
 *
 * 扩展通过工厂函数导出：`export default function(pi: ExtensionAPI) { ... }`
 * 在工厂函数中使用 pi.on()、pi.registerTool()、pi.registerCommand() 等注册能力。
 */
