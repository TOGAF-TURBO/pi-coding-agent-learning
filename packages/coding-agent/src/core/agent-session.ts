/**
 * @fileoverview Agent 会话核心抽象层，管理 Agent 的完整生命周期和会话状态。
 *
 * 本类被所有运行模式（交互式、打印、RPC）共享，封装了：
 *   - Agent 状态的读写访问
 *   - 事件订阅与自动会话持久化
 *   - 模型与思考级别管理
 *   - 对话上下文压缩（手动和自动）
...
export interface AgentSessionConfig {
	...
	/** 资源加载器，用于加载技能、提示模板、主题、对话上下文文件和系统提示 */
	resourceLoader: ResourceLoader;
	/** 通过 SDK 注册的自定义工具（非扩展注册） */
	customTools?: ToolDefinition[];
	/** 模型注册表，用于 API 密钥解析和模型发现 */
	modelRegistry: ModelRegistry;
	/** 初始激活的内置工具名称列表，默认为 [read, bash, edit, write] */
	initialActiveToolNames?: string[];
	/** 工具名称白名单，设置后仅暴露白名单中的工具 */
	allowedToolNames?: string[];
	/**
	 * 覆盖基础工具集（适用于自定义运行时）。
	 *
	 * 内部会合成为最小化的 ToolDefinition，使 AgentSession
	 * 即使在调用方提供纯 AgentTool 实例时也能维持定义优先的工具注册表。
	 */
	baseToolsOverride?: Record<string, AgentTool>;
	/** Agent 用于访问当前 ExtensionRunner 的可变引用 */
	extensionRunnerRef?: { current?: ExtensionRunner };
	/** 扩展绑定到运行时时发出的会话启动事件元数据 */
...
export class AgentSession {
	...
	/** 转向消息模式："all" 发送全部，"one-at-a-time" 逐条发送 */
	get steeringMode(): "all" | "one-at-a-time" {
		return this.agent.steeringMode;
	}

	/** 后续消息模式："all" 发送全部，"one-at-a-time" 逐条发送 */
	...
	/**
	 * 切换模型并触发系统提示重建。
	 * @returns 是否切换成功（可能因无 API Key 失败）
	 */
	 ...
	 * 手动触发上下文压缩。
	 *
	 * 流程：断开 Agent → 中止操作 → 发出 session_before_compact 钩子
	 * → 执行压缩 → 写入会话 → 重建系统提示 → 重连 Agent。
	 *
	 ...
	 * 绑定扩展系统到会话。
	 *
	 * 初始化扩展运行时，加载配置的扩展，注册工具和命令，
	 * 触发 session_start 事件。这是扩展系统激活的关键步骤。
	 */
	 ...
	 * 重新加载扩展、技能、提示模板和主题。
	 *
	 * 触发 session_shutdown 事件、使旧运行时失效、
	 * 重新加载资源并初始化新的扩展运行时。
	 */
