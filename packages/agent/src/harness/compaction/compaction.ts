/**
 * @fileoverview Harness Compaction — Agent 层的上下文压缩实现。
 *
 * 与 coding-agent 层的 compaction 不同，这是 Agent harness 包内的纯函数实现：
 *   - prepareCompaction()：确定压缩点和保留范围
 *   - compact()：调用 LLM 生成摘要
 *   - shouldCompact()：根据 Token 使用率判断是否需要压缩
 *   - estimateContextTokens()：估算当前上下文的 Token 数
 *
 * 压缩流程：将旧消息摘要为一条压缩摘要消息，保留最近消息继续对话。
 */
