...
pub fn get_pricing(model: &str) -> ModelPricing {
    ...
    if model_lower.contains("claude-4-opus") || model_lower.contains("claude-opus-4") {
        return ModelPricing { input_per_m: 15.0, output_per_m: 75.0 };
    }
    if model_lower.contains("claude-4-sonnet") || model_lower.contains("claude-sonnet-4") {
        return ModelPricing { input_per_m: 3.0, output_per_m: 15.0 };
    }
    ...
    if model_lower.contains("claude-3-haiku") {
        return ModelPricing { input_per_m: 0.25, output_per_m: 1.25 };
    }

    // OpenAI GPT
    if model_lower.contains("gpt-4o") && !model_lower.contains("mini") {
        return ModelPricing { input_per_m: 2.5, output_per_m: 10.0 };
    }
    if model_lower.contains("gpt-4o-mini") {
        return ModelPricing { input_per_m: 0.15, output_per_m: 0.6 };
    }
    if model_lower.contains("o3") {
        return ModelPricing { input_per_m: 2.0, output_per_m: 8.0 };
    }
    if model_lower.contains("o4-mini") {
        return ModelPricing { input_per_m: 1.1, output_per_m: 4.4 };
    }

    // Google Gemini
    if model_lower.contains("gemini-2.5-pro") {
        return ModelPricing { input_per_m: 1.25, output_per_m: 10.0 };
    }
    ...
    if model_lower.contains("deepseek-chat") || model_lower.contains("deepseek-v3") {
        return ModelPricing { input_per_m: 0.27, output_per_m: 1.1 };
    }
    if model_lower.contains("deepseek-reasoner") || model_lower.contains("deepseek-r1") {
        return ModelPricing { input_per_m: 0.55, output_per_m: 2.19 };
    }

    // GLM
    if model_lower.contains("glm") {
        return ModelPricing { input_per_m: 0.5, output_per_m: 0.5 };
    }

    // 默认：$3/$15（Claude Sonnet 级别）
    ModelPricing { input_per_m: 3.0, output_per_m: 15.0 }
}
